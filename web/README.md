...
6. **API로 게시** — 승인된 프로필을 Threads Publishing API로 실제 게시합니다. 앱에서 상태와 게시
   번호(seq), 게시물 id, 그리고 게시물을 바로 열어볼 수 있는 "게시글 보기" 링크가 함께 남습니다
   (permalink 조회 자체가 실패해도 게시는 이미 끝난 뒤라 링크만 없이 넘어갑니다). 최초 1회
   `/admin/settings`에서 "Threads 연결"이 필요합니다.
...
6. 관리자 로그인 후 `/admin/settings`의 "Threads 연동"에서 "Threads 연결"을 눌러 OAuth를 완료한다.
   이후 앱이 장기 토큰을 보관·자동 갱신한다. 연결되면 같은 화면에 "테스트 게시"가 나타난다 —
   프로필과 무관하게 문구를 바로 게시해 연결이 실제로 되는지 확인하고, "테스트 글 삭제"로
   지울 수 있다(둘 다 실제로 Threads에 영향을 준다). 게시에 성공하면 "글 보기" 링크도 함께 뜬다.

**이미 연결된 계정이 있는 상태에서 이 저장소를 업데이트했다면 삭제 기능이 "이미 삭제된
게시물입니다" 같은 권한 오류로 실패한다** — `threads_delete` 권한이 나중에 추가돼서,
그 전에 발급된 토큰에는 이 권한이 없다. **연결 해제 → 재연결**하면 새 권한이 포함된
토큰을 다시 받는다.
...
- **"이미 게시가 진행 중입니다" (409)** — 같은 프로필에 게시 요청이 겹쳤거나, 방금 실패한
  시도의 선점이 아직 안 풀린 상태다. 5분 지나면 자동으로 풀리니 그 뒤 다시 시도한다.
- **"게시는 완료됐지만 DB 반영에 실패했습니다 (post id: …)"** — Threads에는 실제로 올라갔는데
  앱 DB 기록만 실패한 상태다. **절대 그대로 다시 누르지 않는다.** 이 경우는 선점을 일부러
  풀지 않지만 5분 뒤에는 똑같이 풀리므로, 그때 다시 누르면 Threads에 같은 글이 한 번 더
  올라간다. 대신:
  1. 오류 메시지의 post id로 Threads에 실제로 올라갔는지 확인한다(서버 로그
     `[threads] 게시 성공 후 DB 반영 실패`에도 같은 id가 남는다).
  2. 이미 올라갔다면 그 프로필 행을 DB에서 직접 고친다 — `status`를 `PUBLISHED`로,
     `publishedPostId`를 그 id로, `seq`를 다음 게시 번호로, `publishStartedAt`을 `NULL`로.
     `publishedPermalink`는 없어도 된다 — 비워 두면 화면에 "게시글 보기" 링크만 안 뜬다.
     이 복구를 관리자 화면에서 할 방법은 아직 없다.
  3. DB를 바로 못 고치는 상황이면 그 행은 그대로 두고(다시 누르지 말고) 게시 사실을 앱
     밖에서 따로 기록해 둔다.
